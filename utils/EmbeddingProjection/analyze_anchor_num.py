    ...
    def fit(self, training_set, val_set, max_epochs=100, batch_size=100, lr=0.01):
        ...
        for epoch in tqdm(range(max_epochs)):
            ...
            for src_batch, tgt_batch in train_loader:
                ...
                # Compute loss (MSE)
                loss = nn.functional.mse_loss(preds, tgt_batch)
                loss.backward()
                optimizer.step()
            ...
            # self.transformation_weights = {"M": M, "b": b}
            # self.transformation_weights = {"M_in": M_in,
            #                             "M_out": M_out,
            #                             "b": b}
...
def parse_args():
    ...
    parser.add_argument('--lr', type=float, default=1.0, help='Learning rate (default: 1)')
...
val_size = 2000
test_size = 2000
all_train_dataset, val_dataset, test_dataset = random_split(dataset, [train_size, val_size, test_size])

for sample_num in [200000]:  #[10000, 40000, 80000, 120000, 160000, 200000]:
    train_dataset = Subset(all_train_dataset, random.sample(range(len(all_train_dataset)), min(sample_num, len(all_train_dataset))))
    ...
    esitmation_embedding_projection.fit(all_train_dataset, anchor_num=sample_num)

    print(f"\nAnchor Num:{sample_num}")
    with torch.no_grad():
